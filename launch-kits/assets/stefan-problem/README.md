...
この小規模パックは、一次元一相融解問題の相似解から作った界面位置表と、その物理・無次元パラメータの整合だけを確認するCPU向け事前診断を提供します。固定格子エンタルピーソルバ、時間積分、界面抽出実装、完成実験、完成図、結論は含みません。
...
事前診断は非対話で、リポジトリへファイルを書きません。エンタルピーソルバを実行せず、学生が選ぶ格子、時間刻み、相変化温度幅も固定しません。
...
このパックは穴埋め式の完成ソルバではありません。問い、比較パラメータ、相変化温度幅、エンタルピー–温度対応、離散化、時間積分、ソルバ構成、API、関数名、内部ファイル構成、界面抽出、評価指標、実験、図表、解釈は各プロジェクトで決めます。まずこの一相ケースで界面位置とエネルギー収支を検証してから発展へ進んでください。
