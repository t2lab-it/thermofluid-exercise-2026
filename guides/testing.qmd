...
このページは最初のbranchとpull request課題（課題ID: F01）を終えた直後の必読資料です。次の配列・関数・テスト課題（課題ID: F02）以降は、課題を始める前にここでテストと数値検証の役割を確認します。
...
まず、最小値と最大値の**符号**、初期値に対する**範囲**、overshoot・undershootの**フラグ**を比べます。小数の完全一致はその後に確認します。これにより、計算方法や境界条件の大きな違いを、丸め誤差の違いと混同しにくくなります。

次の値は、公開baselineの`nx=81`、`dx=0.025`、`cfl=0.5`、`dt=0.0125`、`steps=40`に対応する値です。別のparameterで実行した結果に、この小数をそのまま要求しません。
...
![upwind + Eulerの公開baseline](../assets/n01-reference/upwind.png){fig-alt="初期値とupwind + Eulerの最終値を比較したグラフ"}

![centered + Eulerの公開baseline](../assets/n01-reference/centered-euler.png){fig-alt="初期値とcentered + Eulerの最終値を比較し、振動を示したグラフ"}
...
ここまで確認したら、ページ下部の［次へ］から第3回授業「配列・関数・loop・テスト」へ進んでください。配列・関数・テスト課題のbranchは、授業ページの次にある同課題ページで作成します。
