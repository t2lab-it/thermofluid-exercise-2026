...
| 役割 | 学生リポジトリ内の編集先 |
|---|---|
| 温度の安定刻み・面流束・更新，Burgersの刻み・更新 | [`src/N07Transport.jl`](https://github.com/t2lab-it/thermofluid-exercise-student-2026/blob/059019df9e10ae1b556b78451bbcb092b765dd20/src/N07Transport.jl) |
| 保存区間の熱収支解析 | [`exercises/N07_2d_advection_diffusion/analyze.jl`](https://github.com/t2lab-it/thermofluid-exercise-student-2026/blob/059019df9e10ae1b556b78451bbcb092b765dd20/exercises/N07_2d_advection_diffusion/analyze.jl) |
| 配布済み必須テストと自作2群 | [`exercises/N07_2d_advection_diffusion/tests.jl`](https://github.com/t2lab-it/thermofluid-exercise-student-2026/blob/059019df9e10ae1b556b78451bbcb092b765dd20/exercises/N07_2d_advection_diffusion/tests.jl) |
| 予想・証拠・判断の記録 | [`exercises/N07_2d_advection_diffusion/learning_log.md`](https://github.com/t2lab-it/thermofluid-exercise-student-2026/blob/059019df9e10ae1b556b78451bbcb092b765dd20/exercises/N07_2d_advection_diffusion/learning_log.md) |
| 表示条件 | [`exercises/N07_2d_advection_diffusion/plot.jl`](https://github.com/t2lab-it/thermofluid-exercise-student-2026/blob/059019df9e10ae1b556b78451bbcb092b765dd20/exercises/N07_2d_advection_diffusion/plot.jl) |

N05の周期添字と共通検証を再利用し，N06の完成ソルバは維持します．
時間積分，保存時刻への短縮，HDF5入出力，解析解，基本診断と描画は提供します．
一括入口は`exercises/N07_2d_advection_diffusion/run.jl`です．
`simulate.jl`が計算，`analyze.jl`が解析，`plot.jl`が作図，`run.jl`が一括入口です．
...
Burgersは4配列の全組合せ，流束は入力場および流束同士のaliasを拒否します．
係数は有限，`kappa,nu>=0`，格子幅は正，`0<safety<=1`とし，不正入力は書込み前の`ArgumentError`です．
全係数・速度0では刻み取得を拒否しますが，有限正dtによるstepは旧場をコピーします．
...
再解析・再作図の前後に両HDF5のSHA-256が変わらないことを確認します．
[公式条件](../lessons/N07.qmd#thermal-cases)の温度7ケースと共通刻みの代表比較3ケース，周期Burgersを生成します．
温度周期3ケースとBurgersは$(40,30),(80,60),(160,120)$，他は$(80,60)$，保存時刻は`[0,0.25,0.5,0.75,1]`です．
...
固定温度閉系も解析モードと比べ，開境界へ周期の熱量保存条件を流用しません．
未記入の自作欄は失敗します．
[課題単独と進捗連動のテスト](../guides/workflow.qmd#tests)の両方を実行し，過去課題を含む進捗連動テストでN07は1回だけ実行されます．
...
`diagnostics_complete=true`は診断が揃ったことを表し，精度や収支の合否は必須テストで別に確認します．
未実装，破損データ，混在run_id，古いsummary・図は失敗します．
`simulate.jl`だけを再実行するとsummary・図が古くなるため，再解析・再作図してください．
過去の保存データも解析できますが，現在のコードで計算した結果と来歴を区別します．
失敗時には既存公式出力を保持し，反映と復元の両方が失敗した場合は表示されたバックアップを使います．
学生が追加したファイルも容量へ数えます．
[容量検査](../guides/workflow.qmd#official-results)で1ファイル5 MiB，N07全体10 MiB，全課題100 MiBを守ります．
