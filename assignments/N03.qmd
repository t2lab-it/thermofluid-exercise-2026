...
[N03授業](../lessons/N03.qmd)の[問題・標準条件](../lessons/N03.qmd#problem-setup)を実装し、固定温度と断熱の結果を比較します。
...
必須テストの入力・期待値・判断条件を説明し、削除・無効化せずに保持してください。
次の指定テーマ2つも提出上は必須です。
未記入の`@test false`を自分のテストに置き換えます。
...
各テストの入力、誤差指標、期待値、許容範囲、その根拠と検出できる／できない誤りを学習ログに記します。
...
失敗時は原因を直して再実行し、成功メッセージと出力を確認してください。
...
指定`fo=0.4`と`fo=0.6`を同じ時刻0.1まで計算し、振動・振幅増大を共通の軸で比較します。
`fo`と`t_final`はキーワード引数で変更できます。
非有限値で停止した場合は両計算に共通の`t_final`を短くし、実効`fo`が0.5を超える比較になっているか確認します。
最終時刻への調整で実効値が0.5以下になった場合は、比較条件を調整する必要がある旨のエラーになります。
標準計算の非負性・有界性を、この任意実験の合格条件にはしません。
実験した場合は条件、振動・増幅、初期範囲からの逸脱とテストの適用範囲を学習ログへ追記します。
...
- 配布済み必須4群・指定テーマ自作2つ・過去課題の回帰テストが通り、根拠と限界を説明できる。
