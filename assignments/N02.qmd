...
可視化には初期値`Initial`と最終値`Final`だけでなく，標準条件では`t=0.25，0.50，0.75`の中間時刻も重ねて表示します．
各曲線には時刻順に連続なカラーマップ（`viridis`など）を割り当て，初期条件から最終時刻まで色が連続に変化するようにします．
[公式出力の確認](../guides/workflow.qmd#official-results)を行って3点をcommitします．
実行に失敗した場合は，原因を直して再実行し，成功メッセージと3点の出力を確認してください．

任意の[CFLを変える追加実験](../lessons/N02.qmd#comparison-conditions)では，Juliaで`run.jl`を読み込んで次のように実行できます．
...
![固定境界での非線形移流](../assets/n02-reference/fixed-boundary.png){fig-alt="固定境界，初期から時刻0.25，0.50，0.75，1.0までを連続した色で示した非線形移流の分布．時刻1.0では波の一部が右端から流出し，左側は初期の低い値を保つ"}

![周期境界での非線形移流](../assets/n02-reference/periodic.png){fig-alt="周期境界，初期から時刻0.25，0.50，0.75，1.0までを連続した色で示した非線形移流の分布．時刻1.0では右端を越えた波が左側へ戻り，右端は表示用の先頭値で閉じている"}
...
- 初期・中間・最終時刻の曲線を連続なカラーマップで表示した公式3出力と学習ログを確認し，[学習ログ・diff・commit](../guides/workflow.qmd#commit)から[merge後](../guides/workflow.qmd#after-merge)まで完了した．
